  ...
  const confirmSubmit = async () => {
    try {
      ...
      if (response?.data?.msg) {
        ...
        navigate(`/${portal}/job-listings`, { state: newDataToPass });
      }
    } catch (error) {
      if (error.response) {
            ...
            <div>
              <form onSubmit={handleSubmit}>
                <div className="dashboard-card my-4 rounded-2xl p-6 md:p-8">
                  <div className="mb-6">
                    <p className="dashboard-kicker">Placement Listing</p>
                    <h2 className="mt-2 text-2xl font-extrabold text-slate-950">{jobId ? 'Edit Job Detail' : 'Post New Job'}</h2>
                    <p className="mt-2 text-sm leading-6 text-slate-500">
                      Select the company first, then add role details, eligibility, and application instructions.
                    </p>
                  </div>
                ...
                <div className="dashboard-card my-6 rounded-2xl p-6 md:p-8">
