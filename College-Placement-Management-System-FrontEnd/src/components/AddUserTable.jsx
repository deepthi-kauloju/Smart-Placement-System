...
function AddUserTable({
  ...
  return (
    <>
      <div className='dashboard-card overflow-hidden rounded-2xl bg-white/80 p-3 md:p-5'>
        {
          loading ? (
            ...
            <Table hover responsive className='dashboard-table mb-0 w-full bg-white text-sm'>
              <thead>
                <tr>
                ...
                {users.length > 0 ? (
                  users.map((user, index) => (
                    <tr key={user?.email}>
                      ...
                      <td>
                        <Link to={`mailto:${user.email}`} className='dashboard-table-link'>
                          {user.email}
                        </Link>
                      </td>
                      <td>{user.number}</td>
                        ...
                        <div className="">
                          {
                            userToAdd === 'approve-student' ? (
                              <div className="flex justify-around items-center gap-1 max-sm:flex-col">
                                ...
                                >
                                  <i
                                    className="fa-solid fa-circle-xmark cursor-pointer text-xl text-slate-400 transition hover:text-red-500 max-sm:text-lg"
                                ...
                                >
                                  <i
                                    className="fa-solid fa-square-check cursor-pointer text-xl text-slate-400 transition hover:text-green-500 max-sm:text-lg"
                              ...
                              <div className="">
                                ...
                                >
                                  <i
                                    className="fa-regular fa-trash-can cursor-pointer text-xl text-slate-400 transition hover:text-red-500"
                                    onClick={() => handleDeleteUser(user.email)}
                                    onMouseEnter={(e) => {
                                      e.target.classList.add('fa-solid');
                                      e.target.classList.remove('fa-regular');
                  ...
                  <tr>
                    <td colSpan="6" className="py-8 text-center font-semibold text-slate-500">No users found</td>
                  </tr>
                )}
              </tbody>
            </Table>
