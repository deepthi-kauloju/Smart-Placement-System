...
const normalizeUserData = (data = {}) => ({
  ...
  middle_name: cleanValue(data.middle_name),
  ...
  profile: cleanValue(data.profile) || '/profileImgs/default/defaultProfileImg.jpg',
  fullAddress: {
    address: cleanValue(data.fullAddress?.address),
    pincode: cleanValue(data.fullAddress?.pincode),
  },
  studentProfile: {
    ...data.studentProfile,
    UIN: cleanValue(data.studentProfile?.UIN || data.studentProfile?.uin),
    uin: cleanValue(data.studentProfile?.uin || data.studentProfile?.UIN),
    rollNumber: cleanValue(data.studentProfile?.rollNumber),
    department: cleanValue(data.studentProfile?.department),
    year: cleanValue(data.studentProfile?.year),
    addmissionYear: cleanValue(data.studentProfile?.addmissionYear),
    gap: data.studentProfile?.gap,
    liveKT: cleanValue(data.studentProfile?.liveKT),
    resume: {
      filename: cleanValue(data.studentProfile?.resume?.filename),
      filepath: cleanValue(data.studentProfile?.resume?.filepath),
      contentType: cleanValue(data.studentProfile?.resume?.contentType),
    },
    SGPA: {
      sem1: cleanValue(data.studentProfile?.SGPA?.sem1),
      sem2: cleanValue(data.studentProfile?.SGPA?.sem2),
      sem3: cleanValue(data.studentProfile?.SGPA?.sem3),
      sem4: cleanValue(data.studentProfile?.SGPA?.sem4),
      sem5: cleanValue(data.studentProfile?.SGPA?.sem5),
      sem6: cleanValue(data.studentProfile?.SGPA?.sem6),
      sem7: cleanValue(data.studentProfile?.SGPA?.sem7),
      sem8: cleanValue(data.studentProfile?.SGPA?.sem8),
    },
    pastQualification: {
      ssc: {
        board: cleanValue(data.studentProfile?.pastQualification?.ssc?.board),
        percentage: cleanValue(data.studentProfile?.pastQualification?.ssc?.percentage),
        year: cleanValue(data.studentProfile?.pastQualification?.ssc?.year),
      },
      hsc: {
        board: cleanValue(data.studentProfile?.pastQualification?.hsc?.board),
        percentage: cleanValue(data.studentProfile?.pastQualification?.hsc?.percentage),
        year: cleanValue(data.studentProfile?.pastQualification?.hsc?.year),
      },
      diploma: {
        board: cleanValue(data.studentProfile?.pastQualification?.diploma?.board),
        department: cleanValue(data.studentProfile?.pastQualification?.diploma?.department),
        percentage: cleanValue(data.studentProfile?.pastQualification?.diploma?.percentage),
        year: cleanValue(data.studentProfile?.pastQualification?.diploma?.year),
      },
    },
  },
});
...
const getDisplayName = (data) => (
  [cleanValue(data?.first_name), cleanValue(data?.middle_name), cleanValue(data?.last_name)]
    .filter(Boolean)
    .join(' ')
);
...
function UserDetails() {
        ...
        if (response.data.msg) {
          ...
          if (completeProfileReq) {
            if (response.data.msg === "Data Updated Successfully!")
              if (currentUserData.role === "student") navigate('../student/dashboard');
              if (currentUserData.role === "tpo_admin") navigate('../tpo/dashboard');
              if (currentUserData.role === "management_admin") navigate('../management/dashboard');
          }
        }
  ...
  const handlePhotoChange = async (e) => {
    ...
    if (file) {
      ...
      formData.append('userId', userData?._id || userId || currentUserData?.id);
      ...
      try {
        ...
        setUserData((prev) => normalizeUserData({ ...prev, profile: response.data.file }));
        if (response.data) {
          if (response.data.msg) {
            setToastMessage(response.data.msg);
            setShowToast(true);
          ...
          <>
            ...
            <div className='px-4 py-10'>
              <h1 className='text-2xl md:text-4xl'>
                {getDisplayName(userData) || 'Complete Your Profile'}
              </h1>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div className="px-2 py-3 flex flex-col items-center gap-4 my-1">
                      <Col className="w-32 md:w-44 lg:w-52">
                        <Image src={FILE_BASE_URL + userData?.profile} roundedCircle />
                      </Col>
